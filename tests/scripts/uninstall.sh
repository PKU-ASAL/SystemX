#!/bin/bash
# SysArmor 终端卸载脚本 (无代理模式)
# 自动生成，收集器ID: 5ff4f634-9928-4db3-a31f-6abcef64c75e
# 生成时间: 2025-09-01 02:22:05 UTC
# 指令: curl -s "http://localhost:8080/api/v1/scripts/uninstall-terminal.sh?collector_id=5ff4f634-9928-4db3-a31f-6abcef64c75e"
# 脚本功能说明:
# ============
#
# 1. 预检查阶段 (Pre-uninstall Checks)
#    - 检查 root 权限：确保有足够权限修改系统配置
#    - 安装状态检查：验证 SysArmor 是否已安装
#    - 收集器ID验证：确保卸载正确的收集器实例
#
# 2. 备份阶段 (Backup Phase)
#    - 备份当前配置：保存所有 SysArmor 相关配置文件
#    - 创建时间戳备份目录：/tmp/sysarmor-uninstall-backup-YYYYMMDD-HHMMSS
#    - 支持配置恢复和问题排查
#
# 3. 服务停止阶段 (Service Shutdown Phase)
#    - 停止 rsyslog：断开到 Vector 的数据连接
#    - 验证连接断开：确认没有活跃的网络连接
#    - 停止 auditd：停止审计事件收集
#    - 等待服务完全停止
#
# 4. 配置清理阶段 (Configuration Cleanup Phase)
#    - 删除 rsyslog 配置：/etc/rsyslog.d/99-sysarmor.conf
#    - 删除 audit 规则：/etc/audit/rules.d/sysarmor.rules
#    - 清理队列文件：/var/spool/rsyslog/sysarmor_queue*
#    - 删除配置目录：/etc/sysarmor/
#
# 5. 系统恢复阶段 (System Restoration Phase)
#    - 恢复默认 auditd 配置：重置为系统默认设置
#    - 清理当前审计规则：移除所有 SysArmor 规则
#    - 重新生成规则：使用 augenrules --load 重新加载
#
# 6. 服务重启阶段 (Service Restart Phase)
#    - 启动 auditd：恢复系统审计功能
#    - 启动 rsyslog：恢复系统日志功能
#    - 等待服务稳定：确保服务正常运行
#
# 7. 连接验证阶段 (Connection Verification Phase)
#    - 检查残留连接：确认没有到 Vector 的活跃连接
#    - 自动重启处理：如果发现连接，自动重启 rsyslog 断开
#    - 最终验证：确保连接完全断开
#
# 8. 完整性验证阶段 (Integrity Verification Phase)
#    - 服务状态检查：确认 auditd 和 rsyslog 运行正常
#    - 配置文件检查：确认所有 SysArmor 文件已删除
#    - 审计规则检查：确认没有残留的 SysArmor 规则
#    - 生成卸载报告：提供验证命令和故障排除信息
#
# 关键修复说明:
# ============
# - 自动连接断开：检测到残留连接时自动重启 rsyslog
# - 简化验证逻辑：移除冗长的循环等待，改为一次性检查
# - 错误处理优化：修复整数比较 bug，改进错误提示
# - 用户体验改进：减少等待时间，提供清晰的操作反馈
#
# 数据流向清理:
# ============
# 停止: audit 事件 ❌ /var/log/audit/audit.log ❌ rsyslog ❌ Vector
# 清理: 配置文件、队列文件、规则文件、元数据文件
# 恢复: 默认系统配置，正常的审计和日志功能
#
# 安全考虑:
# ========
# - 完整清理：确保没有残留的监控配置
# - 系统稳定：保证卸载后系统服务正常
# - 数据保护：备份所有配置以防误操作
# - 连接安全：确保没有未授权的数据传输

set -e

# 配置变量
COLLECTOR_ID="5ff4f634-9928-4db3-a31f-6abcef64c75e"
WORKER_HOST="middleware-vector"
WORKER_PORT="6000"
CONFIG_DIR="/etc/sysarmor"
RSYSLOG_CONFIG="/etc/rsyslog.d/99-sysarmor.conf"
AUDIT_RULES="/etc/audit/rules.d/sysarmor.rules"

echo "🗑️  SysArmor 卸载开始..."
echo "收集器ID: $COLLECTOR_ID"
echo ""

# 权限检查
if [[ $EUID -ne 0 ]]; then
   echo "❌ 请使用 root 权限运行此脚本 (使用 sudo)"
   exit 1
fi

# 安装检查
if [ ! -f "$CONFIG_DIR/collector_id" ]; then
    echo "ℹ️  未发现 SysArmor 收集器"
    echo "✅ 系统已经是干净状态"
    exit 0
fi

# ID 验证
INSTALLED_ID=$(cat "$CONFIG_DIR/collector_id" 2>/dev/null || echo "")
if [ "$INSTALLED_ID" != "$COLLECTOR_ID" ]; then
    echo "⚠️  收集器ID不匹配:"
    echo "    期望: $COLLECTOR_ID"
    echo "    发现: $INSTALLED_ID"
    echo "❌ 此卸载脚本用于不同的收集器"
    exit 1
fi

# 备份配置
echo "💾 备份当前配置..."
BACKUP_DIR="/tmp/sysarmor-uninstall-backup-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP_DIR"
for file in "$RSYSLOG_CONFIG" "$AUDIT_RULES" "/etc/audit/auditd.conf" "$CONFIG_DIR"; do
    if [ -e "$file" ]; then
        cp -r "$file" "$BACKUP_DIR/"
        echo "  - 已备份 $(basename $file)"
    fi
done

# 关键修复：先停止 rsyslog 断开连接
echo "🛑 停止服务并断开连接..."
sudo systemctl stop rsyslog
echo "  - 已停止 rsyslog"
sleep 2

# 验证连接断开
echo "🔌 验证连接断开..."
ACTIVE_CONN=$(sudo ss -tupn | grep ":$WORKER_PORT" | grep rsyslog | wc -l)
if [ "$ACTIVE_CONN" -eq 0 ]; then
    echo "  ✅ rsyslog 连接已断开"
else
    echo "  ⚠️  检测到 $ACTIVE_CONN 个活跃连接"
fi

# 停止 auditd
sudo systemctl stop auditd
echo "  - 已停止 auditd"

# 清理配置文件
echo "🧹 清理配置文件..."
if [ -f "$RSYSLOG_CONFIG" ]; then
    sudo rm -f "$RSYSLOG_CONFIG"
    echo "  - 已删除 rsyslog 配置"
fi

if [ -f "$AUDIT_RULES" ]; then
    sudo rm -f "$AUDIT_RULES"
    echo "  - 已删除 audit 规则"
fi

# 清理队列文件
if [ -d "/var/spool/rsyslog" ]; then
    sudo rm -f /var/spool/rsyslog/sysarmor_queue* 2>/dev/null || true
    echo "  - 已清理队列文件"
fi

# 清理配置目录
if [ -d "$CONFIG_DIR" ]; then
    sudo rm -rf "$CONFIG_DIR"
    echo "  - 已删除配置目录"
fi

# 恢复默认 auditd 配置
echo "⚙️ 恢复默认配置..."
sudo tee /etc/audit/auditd.conf > /dev/null << 'EOF'
# 默认 auditd 配置
# 由 SysArmor 卸载脚本恢复

log_file = /var/log/audit/audit.log
log_format = RAW
log_group = adm
priority_boost = 4
flush = INCREMENTAL_ASYNC
freq = 50

max_log_file = 100
num_logs = 5
max_log_file_action = ROTATE

space_left = 75
space_left_action = SYSLOG
admin_space_left = 50
admin_space_left_action = SUSPEND
disk_full_action = SUSPEND
disk_error_action = SUSPEND

tcp_listen_queue = 5
tcp_max_per_addr = 1
tcp_client_ports = 1024-65535
tcp_client_max_idle = 0

name_format = HOSTNAME
plugin_dir = /etc/audit/plugins.d
distribute_network = no
write_logs = yes
EOF

# 清理 audit 规则
echo "🔄 清理审计规则..."
sudo auditctl -D 2>/dev/null || true
sudo augenrules --load

# 关键修复：重启服务并确保连接断开
echo "🔄 重启服务..."

# 处理auditd服务启动 - 某些系统不允许直接重启
echo "  - 启动auditd服务..."
if sudo systemctl start auditd 2>/dev/null; then
    echo "    ✅ auditd启动成功"
else
    echo "    ⚠️  auditd无法通过systemctl启动，尝试替代方法..."
    # 尝试传统服务命令
    if sudo service auditd start 2>/dev/null || sudo /sbin/service auditd start 2>/dev/null; then
        echo "    ✅ auditd启动成功"
    else
        echo "    ⚠️  auditd启动失败，可能需要重启系统"
        echo "    💡 建议: 重启系统后auditd将自动启动"
    fi
fi

sleep 2

# 启动rsyslog服务
echo "  - 启动rsyslog服务..."
if sudo systemctl start rsyslog 2>/dev/null; then
    echo "    ✅ rsyslog启动成功"
else
    echo "    ⚠️  rsyslog启动可能有问题"
fi

sleep 3

# 最终验证连接断开
echo "🔍 最终验证..."
UNINSTALL_SUCCESS=true

# 检查服务状态
for service in auditd rsyslog; do
    if sudo systemctl is-active --quiet $service; then
        echo "  ✅ $service 运行正常"
    else
        echo "  ⚠️  $service 可能有问题"
        UNINSTALL_SUCCESS=false
    fi
done

# 检查配置文件清理
for file in "$RSYSLOG_CONFIG" "$AUDIT_RULES" "$CONFIG_DIR"; do
    if [ -e "$file" ]; then
        echo "  ⚠️  $file 仍然存在"
        UNINSTALL_SUCCESS=false
    fi
done

# 关键验证：检查是否还有活跃连接
RSYSLOG_CONN=$(sudo ss -tupn | grep ":$WORKER_PORT" | grep rsyslog | wc -l)
if [ "$RSYSLOG_CONN" -eq 0 ]; then
    echo "  ✅ 确认无活跃连接"
else
    echo "  ⚠️  仍有 $RSYSLOG_CONN 个连接到 $WORKER_HOST:$WORKER_PORT"
    echo "  🔄 正在重启 rsyslog 断开连接..."
    sudo systemctl restart rsyslog
    sleep 3
    
    # 再次验证连接
    FINAL_CONN=$(sudo ss -tupn | grep ":$WORKER_PORT" | grep rsyslog | wc -l)
    if [ "$FINAL_CONN" -eq 0 ]; then
        echo "  ✅ 连接已成功断开"
    else
        echo "  ⚠️  连接仍然存在，可能需要手动处理"
        UNINSTALL_SUCCESS=false
    fi
fi

# 检查 SysArmor 相关规则 (修复整数比较bug)
SYSARMOR_RULES=$(sudo auditctl -l 2>/dev/null | grep -E "(suspicious_activity|privilege_escalation|sensitive_files|system_binaries|network_connections|authentication|scheduled_tasks)" | wc -l)
if [ "$SYSARMOR_RULES" -gt 0 ]; then
    echo "  ⚠️  仍有 $SYSARMOR_RULES 条 SysArmor 规则"
fi

# 最终结果
if [ "$UNINSTALL_SUCCESS" = true ]; then
    echo ""
    echo "🎉 SysArmor 卸载成功！"
    echo ""
    echo "📋 卸载摘要:"
    echo "  - 收集器ID: $COLLECTOR_ID"
    echo "  - rsyslog 配置: 已删除"
    echo "  - audit 规则: 已删除"
    echo "  - 配置目录: 已删除"
    echo "  - 连接状态: 已断开"
    echo "  - 备份位置: $BACKUP_DIR"
    echo ""
    echo "🔍 验证命令:"
    echo "  sudo systemctl status rsyslog auditd"
    echo "  sudo ss -tupn | grep :$WORKER_PORT"
    echo "  sudo auditctl -l"
else
    echo ""
    echo "⚠️  卸载完成但存在警告，请检查上述问题"
    echo ""
    echo "💡 如果连接仍然存在，请手动重启 rsyslog:"
    echo "    sudo systemctl restart rsyslog"
fi

echo ""
echo "⚠️  重要提醒:"
echo "   - 请在 SysArmor 管理界面删除此收集器记录"
echo "   - 配置备份保存在: $BACKUP_DIR"