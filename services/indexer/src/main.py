...
class SysArmorIndexer:
            ...
            if template_file.endswith('.json'):
                ...
                try:
                    ...
                    if 'alerts' in template_name:
                        if 'audit' in template_name:
                            self.create_audit_alerts_index()
                        else:
    ...
    def create_audit_alerts_index(self):
        """创建audit告警索引"""
        try:
            index_name = "sysarmor-alerts-audit"
            ...
            if not self.opensearch_client.indices.exists(index=index_name):
                ...
                logger.info(f"Created audit alerts index: {index_name}")
            else:
                logger.info(f"Audit alerts index already exists: {index_name}")
                
        except Exception as e:
            logger.error(f"Failed to create audit alerts index: {e}")
