...
这些脚本验证 v3 控制面第一阶段:规则内容和策略能被 manager 管理、版本化、发布、分配和审计,并影响 endpoint/cloud analytics。draft policy 可以保存和查询,但不会被分配或成为 effective policy。
...
| `e2e-postgres-agent-projection` | Go backend adapter test | snapshot-backed Postgres 保存时会同步 upsert `agents` / `agent_health` 表投影 |
| `e2e-postgres-ingest-projection` | Go backend adapter test | snapshot-backed Postgres 保存时会同步 upsert `events` / `signals` 表投影 |
| `e2e-postgres-event-query` | Go backend adapter test | Postgres backend 的 `ListEvents` 会优先从 `events` 表读出 event JSON |
| `e2e-postgres-signal-query` | Go backend adapter test | Postgres backend 的 `ListSignals` 会优先从 `signals` 表读出 signal JSON |
| `e2e-postgres-incident-query` | Go backend adapter test | Postgres backend 的 `ListIncidents` 会优先从 `incidents` 表读出 incident JSON |
| `e2e-postgres-control-projection` | Go backend adapter test | snapshot-backed Postgres 保存时会同步 upsert `rules` / `evidence_pullbacks` 表投影 |
| `e2e-postgres-control-audit-projection` | Go backend adapter test | snapshot-backed Postgres 保存时会同步 upsert `policy_audit` / `operator_role_bindings` 表投影 |
| `e2e-postgres-response-projection` | Go backend adapter test | snapshot-backed Postgres 保存时会同步 upsert `response_audit` 表投影,包含 command 与 ack |
| `e2e-postgres-response-write` | Go backend adapter test | Postgres backend 创建/确认 response 时不依赖 snapshot save,会直接 upsert `response_audit` 表 |
| `e2e-postgres-response-query` | Go backend adapter test | Postgres backend 的 `ListResponses` 会优先从 `response_audit` 表读出 command/ack 审计记录 |
| `e2e-postgres-policy-projection` | Go backend adapter test | snapshot-backed Postgres 保存时会同步 upsert `policies` / `policy_assignments` 表投影 |
| `e2e-postgres-policy-write` | Go backend adapter test | Postgres backend 创建/发布/分配 policy 和记录 audit 时不依赖 snapshot save,会直接 upsert policy 控制面表 |
| `e2e-postgres-policy-query` | Go backend adapter test | Postgres backend 的 `ListPolicies` / `ListAssignments` 会优先从控制面表读出 policy/assignment JSON |
| `e2e-postgres-policy-get` | Go backend adapter test | Postgres backend 的 `GetPolicy` 会优先按 id/version 从 `policies` 表读出 policy JSON |
...
| Postgres agents/agent_health table projection | `e2e-postgres-agent-projection` | 部分覆盖 |
| Postgres events/signals table projection | `e2e-postgres-ingest-projection` | 部分覆盖 |
| Postgres events table query path | `e2e-postgres-event-query` | 部分覆盖 |
| Postgres signals table query path | `e2e-postgres-signal-query` | 部分覆盖 |
| Postgres incidents table query path | `e2e-postgres-incident-query` | 部分覆盖 |
| Postgres rules/evidence_pullbacks table projection | `e2e-postgres-control-projection` | 部分覆盖 |
| Postgres policy_audit/operator_role_bindings table projection | `e2e-postgres-control-audit-projection` | 部分覆盖 |
| Postgres response_audit table projection | `e2e-postgres-response-projection` | 部分覆盖 |
| Postgres response_audit table write path | `e2e-postgres-response-write` | 部分覆盖 |
| Postgres response_audit table query path | `e2e-postgres-response-query` | 部分覆盖 |
| Postgres policies/policy_assignments table projection | `e2e-postgres-policy-projection` | 部分覆盖 |
| Postgres policy control table write path | `e2e-postgres-policy-write` | 部分覆盖 |
| Postgres policies/policy_assignments table query path | `e2e-postgres-policy-query` | 部分覆盖 |
| Postgres GetPolicy table query path | `e2e-postgres-policy-get` | 部分覆盖 |
