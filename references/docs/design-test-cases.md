...
| `e2e-postgres-agent-health-projection` | Go backend adapter test | snapshot-backed Postgres 保存时会同步 upsert `agent_health` 表投影 |
...
| Postgres agent_health table projection | `e2e-postgres-agent-health-projection` | 部分覆盖 |
