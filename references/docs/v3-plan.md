...
- 逐表 Postgres adapter:当前只开始投影 `agents` / `agent_health` / `events` / `signals` / `rules` / `response_audit` / `policies` / `policy_assignments` / `policy_audit` / `operator_role_bindings` / `incidents` / `incident_events` / `evidence` / `evidence_pullbacks` / `link1_sessions` / `rarity_baseline` / `metrics`,尚未把 ingest/query/policy/incident 主路径迁到逐表读写。
