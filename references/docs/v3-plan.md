...
- Postgres snapshot adapter 已开始接入逐表读路径:当前 `ListEvents` / `ListSignals` / `ListIncidents` 会优先从 `events` / `signals` / `incidents` 表读取,失败或无结果时回退到 snapshot 内存态;policy / response 等主读写路径仍待迁移。
...
- 逐表 Postgres adapter:当前已开始投影 `agents` / `agent_health` / `events` / `signals` / `rules` / `response_audit` / `policies` / `policy_assignments` / `policy_audit` / `operator_role_bindings` / `incidents` / `incident_events` / `evidence` / `evidence_pullbacks` / `link1_sessions` / `rarity_baseline` / `metrics`,并已接入 `events` / `signals` / `incidents` 表读路径;尚未把 policy/response 等主路径完整迁到逐表读写。
