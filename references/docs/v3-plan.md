...
- `internal/store/migrations` 定义 Postgres schema v1,覆盖 agents、agent_health、rules、policies、policy_assignments、policy_audit、operator_role_bindings、events、signals、incidents、incident_events、evidence、response_audit、evidence_pullbacks、link1_sessions、rarity_baseline、metrics 和基础查询索引。
...
- Postgres snapshot adapter 保存 snapshot 时会同步投影 agents 到 `agents` 表、agent health 到 `agent_health` 表,events 到 `events` 表,signals 到 `signals` 表,rules 到 `rules` 表,response command/ack 到 `response_audit` 表,policy versions 到 `policies` 表,policy assignments 到 `policy_assignments` 表,policy audit 到 `policy_audit` 表,operator role bindings 到 `operator_role_bindings` 表,incidents 到 `incidents` 表,incident/event references 到 `incident_events` 表,incident evidence nodes/edges 到 `evidence` 表,evidence pullback requests 到 `evidence_pullbacks` 表,Link1 sessions/cursors 到 `link1_sessions` 表,rarity workload/signal counts 到 `rarity_baseline` 表,manager metrics 到 `metrics` 表,作为逐表 adapter 的第一组运营表路径。
...
- Postgres snapshot adapter 已开始接入逐表读写路径:当前 `ListEvents` / `ListSignals` / `ListIncidents` / `ListResponses` / `ListPolicies` / `GetPolicy` / `ListAssignments` / `EffectivePolicy` 会优先从 `events` / `signals` / `incidents` / `response_audit` / `policies` / `policy_assignments` 表读取,失败或无结果时回退到 snapshot 内存态;response command / ack / approval mutation 会同步 upsert `response_audit` 表,policy upsert/publish/assignment/audit mutation 会同步 upsert `policies` / `policy_assignments` / `policy_audit` 表。
...
- `make -C test e2e-postgres-event-query` 验证 Postgres backend 查询 events 时会走 `events` 表读路径。
- `make -C test e2e-postgres-signal-query` 验证 Postgres backend 查询 signals 时会走 `signals` 表读路径。
- `make -C test e2e-postgres-incident-query` 验证 Postgres backend 查询 incidents 时会走 `incidents` 表读路径。
- `make -C test e2e-postgres-control-projection` 验证 Postgres backend 保存 snapshot 时会 upsert `rules` 和 `evidence_pullbacks` 表投影。
- `make -C test e2e-postgres-control-audit-projection` 验证 Postgres backend 保存 snapshot 时会 upsert `policy_audit` 和 `operator_role_bindings` 表投影。
...
- `make -C test e2e-postgres-response-write` 验证 Postgres backend 创建/确认 response 时不依赖 snapshot save,会直接 upsert `response_audit` 表。
...
- `make -C test e2e-postgres-policy-write` 验证 Postgres backend 创建/发布/分配 policy 和记录 audit 时不依赖 snapshot save,会直接 upsert policy 控制面表。
...
- `make -C test e2e-postgres-effective-policy-query` 验证 Postgres backend 计算 effective policy 时会走 `policy_assignments` / `policies` 表读路径。
...
- `make -C test e2e-postgres-rarity-projection` 验证 Postgres backend 保存 snapshot 时会 upsert `rarity_baseline` 表投影。
...
- 逐表 Postgres adapter:当前已开始投影 `agents` / `agent_health` / `events` / `signals` / `rules` / `response_audit` / `policies` / `policy_assignments` / `policy_audit` / `operator_role_bindings` / `incidents` / `incident_events` / `evidence` / `evidence_pullbacks` / `link1_sessions` / `rarity_baseline` / `metrics`,并已接入 `events` / `signals` / `incidents` / `response_audit` / `policies` / `policy_assignments` 表读路径;response audit 和 policy 控制面已有第一组 mutation 写路径;完整 manager API 级逐表 Postgres adapter e2e 仍待补齐。
...
make -C test e2e-postgres-event-query
make -C test e2e-postgres-signal-query
make -C test e2e-postgres-incident-query
make -C test e2e-postgres-control-projection
make -C test e2e-postgres-control-audit-projection
...
make -C test e2e-postgres-effective-policy-query
...
make -C test e2e-postgres-rarity-projection
```
