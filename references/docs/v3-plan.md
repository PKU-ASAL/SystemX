...
- Postgres snapshot adapter 保存 snapshot 时会同步投影 agents 到 `agents` 表、agent health 到 `agent_health` 表,response command/ack 到 `response_audit` 表,policy versions 到 `policies` 表,policy assignments 到 `policy_assignments` 表,incidents 到 `incidents` 表,incident evidence nodes/edges 到 `evidence` 表,作为逐表 adapter 的第一组运营表路径。
...
- 逐表 Postgres adapter:当前只开始投影 `agents` / `agent_health` / `response_audit` / `policies` / `policy_assignments` / `incidents` / `evidence`,尚未把 ingest/query/policy/incident 主路径迁到逐表读写。
