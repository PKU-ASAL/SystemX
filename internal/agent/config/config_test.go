...
`)
	...
	for _, preserved := range [][]byte{
		[]byte("label.owner: platform"),
		[]byte("selector: exact-container"), []byte("max_batch_items: 17"),
		[]byte(`path: "/custom/content"`),
	} {
		if !bytes.Contains(merged, preserved) {
			t.Fatalf("merged config missing preserved value %q:\n%s", preserved, merged)
		}
	}
	for _, replaced := range [][]byte{
		[]byte(`default_path: "/opt/sysarmor/agent/content/default"`),
		[]byte(`trust_keys: "release=new-key"`),
	} {
