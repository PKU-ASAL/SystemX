...
SYSARMOR_OUTAGE_EVENTS="$OUTAGE_EVENTS" bash "$ROOT/test/e2e/reliability/e2e-outage-soak.sh"

echo "[e2e-agent-reliability-soak] sensor restart recovers after failure"
bash "$ROOT/test/e2e/agent-runtime/e2e-sensor-restart.sh"
bash "$ROOT/test/e2e/agent-runtime/e2e-sensor-recover.sh"

echo "[e2e-agent-reliability-soak] graceful shutdown drains queued batch"
bash "$ROOT/test/e2e/reliability/e2e-shutdown.sh"
