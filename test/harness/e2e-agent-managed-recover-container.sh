...
WORK="/tmp/sysarmor-agent-managed-recover-container"

mkdir -p "$RESULTS"

cleanup() {
  docker exec mgr sh -c 'if [ -f /tmp/sysarmor-agent-managed-recover-container/agent.pid ]; then kill "$(cat /tmp/sysarmor-agent-managed-recover-container/agent.pid)" 2>/dev/null || true; fi' >/dev/null 2>&1 || true
}
trap cleanup EXIT

echo "[e2e-agent-managed-recover-container] starting container topology"
...
if [ \"\$1\" = \"getevents\" ]; then
  while true; do
    printf '%s\n' '{\"process_exec\":{\"process\":{\"pid\":220,\"uid\":0,\"binary\":\"/bin/bash\",\"arguments\":\"-c id\",\"start_time\":\"2026-06-14T10:00:00Z\"},\"parent\":{\"pid\":1,\"binary\":\"/sbin/init\",\"start_time\":\"2026-06-14T09:59:59Z\"}},\"node_name\":\"container-host\",\"time\":\"2026-06-14T10:00:00Z\"}'
    sleep 0.1
  done
fi
EOF
...
{
  \"version\": \"container-managed-recover\",
  \"files\": {
...
agent:
  id: container-agent-managed-recover
...
wait_contains() {
  ...
  until "$@" >"$out" 2>"$out.err" && grep -Fq "$needle" "$out"; do
    if (( SECONDS >= deadline )); then
      echo "[e2e-agent-managed-recover-container][ERROR] timeout waiting for $needle via $cmd_name" >&2
...
docker exec mgr cat "$WORK/agent.log" > "$RESULTS/e2e-agent-managed-recover-container.agent.log"

echo "[e2e-agent-managed-recover-container] ok"
