...
BIN="$ROOT/bin"
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sysarmor-link1-frames.XXXXXX")"
...
echo "[e2e-link1-frames] building binaries"
...
wait_contains() {
  ...
  until "$@" >"$out" && grep -Fq "$needle" "$out"; do
    if (( SECONDS >= deadline )); then
      echo "[e2e-link1-frames][ERROR] $name missing $needle" >&2
...
[
  {
    "type": "upload",
    "payload": {
      "batch_id": "link1-frame-batch",
      "agent": {
        "agent_id": "link1-frame-agent",
        "host_id": "link1-frame-host",
        "tenant_id": "default",
        "version": "e2e"
      },
      "events": [
        {"id": "ev-link1-frame", "scenario": "link1-frame", "kind": "EVENT_KIND_EXEC"}
      ]
    }
  },
  {
...
"$BIN/sysarmorctl" --mgr "$MGR_URL" --json link1-sessions \
  --tenant-id default \
  --agent-id link1-frame-agent > "$RESULTS/e2e-link1-frames.sessions.json"
for want in '"last_ack_cursor":"link1-frame-batch"' '"transport":"frame"'; do
  if ! grep -Fq "$want" "$RESULTS/e2e-link1-frames.sessions.json"; then
    echo "[e2e-link1-frames][ERROR] session missing $want" >&2
    cat "$RESULTS/e2e-link1-frames.sessions.json" >&2
    exit 1
  fi
done
...
"$BIN/sysarmorctl" --mgr "$MGR_URL" --json agent-health \
  --tenant-id default \
  --agent-id link1-frame-agent > "$RESULTS/e2e-link1-frames.health.json"
if ! grep -Fq '"status":"ok"' "$RESULTS/e2e-link1-frames.health.json"; then
  echo "[e2e-link1-frames][ERROR] health frame was not applied" >&2
  cat "$RESULTS/e2e-link1-frames.health.json" >&2
  exit 1
fi

"$BIN/sysarmorctl" --mgr "$MGR_URL" --json responses \
  --tenant-id default \
  --agent-id link1-frame-agent > "$RESULTS/e2e-link1-frames.responses.json"
if ! grep -Fq '"ack":{"response_id":"resp-link1-frame"' "$RESULTS/e2e-link1-frames.responses.json"; then
  echo "[e2e-link1-frames][ERROR] ack frame was not persisted" >&2
