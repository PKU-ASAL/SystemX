...
WORKLOAD="${SCENARIO:-${SYSARMOR_DIAG_WORKLOAD:-mixed-edr-storm}}"
...
if [[ "$MODE" == "workload" && ! -f "$ROOT/workloads/vm/$WORKLOAD/run.sh" && ! -f "$ROOT/scenarios/vm/$WORKLOAD/attack.sh" ]]; then
...
if [[ "$MODE" == "workload" ]]; then
  if [[ -f "$ROOT/workloads/vm/$WORKLOAD/run.sh" ]]; then
    vagrant upload "$ROOT/workloads/vm/$WORKLOAD/run.sh" /tmp/sysarmor-diag-workload.sh node-a >/dev/null
  else
    vagrant ssh node-a -c "sudo cp '/vagrant/test/scenarios/vm/$WORKLOAD/attack.sh' /tmp/sysarmor-diag-workload.sh && sudo chmod +x /tmp/sysarmor-diag-workload.sh" >/dev/null
  fi
fi
