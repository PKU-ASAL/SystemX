...
cat > "$TMP/batch.json" <<JSON
{
  ...
  "events": [
    {"sequence": 1, "event": {"id": "ev-page-1", "agentId": "query-pagination-agent", "hostId": "query-pagination-host", "tenantId": "default", "labels": {"scenario": "$SCENARIO"}, "behavior": "process.exec"}},
    {"sequence": 2, "event": {"id": "ev-page-2", "agentId": "query-pagination-agent", "hostId": "query-pagination-host", "tenantId": "default", "labels": {"scenario": "$SCENARIO"}, "behavior": "file.open"}},
    {"sequence": 3, "event": {"id": "ev-page-3", "agentId": "query-pagination-agent", "hostId": "query-pagination-host", "tenantId": "default", "labels": {"scenario": "$SCENARIO"}, "behavior": "network.connect"}}
  ],
  "signals": [
    {
      "sequence": 1,
...
"$BIN/sysarmorctl" --manager-url "$MGR_URL" --json manager events list \
  --label scenario="$SCENARIO" --limit 1 --offset 1 > "$RESULTS/e2e-query-pagination.events.json"
...
"$BIN/sysarmorctl" --manager-url "$MGR_URL" --json manager signals list \
  --label scenario="$SCENARIO" --limit 1 --offset 1 > "$RESULTS/e2e-query-pagination.signals.json"
