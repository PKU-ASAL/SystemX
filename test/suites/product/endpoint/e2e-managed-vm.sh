...
sensor:
  backend: tetragon
  mode: managed
  bundle_dir: /opt/sysarmor/agent/bundles/tetragon-vm
...
vagrant ssh node-a -c "sudo systemctl stop sysarmor-agent 2>/dev/null || true; sudo rm -rf /opt/sysarmor/agent/bundles/tetragon-vm /opt/sysarmor/agent/sensors/tetragon/vm-managed /opt/sysarmor/agent/sensors/tetragon/current /var/lib/sysarmor/agent/telemetry /tmp/sysarmor-managed-vm-tetragon-count; sudo mkdir -p /opt/sysarmor/agent/bin /opt/sysarmor/agent/bundles/tetragon-vm/bin /etc/sysarmor/policies /var/lib/sysarmor/agent/telemetry; sudo install -m 0755 /tmp/sysarmor-agent.upload /opt/sysarmor/agent/bin/sysarmor-agent; sudo install -m 0644 /tmp/sysarmor-agent.service.upload /etc/systemd/system/sysarmor-agent.service; sudo install -m 0755 /tmp/sysarmor-fake-tetragon /opt/sysarmor/agent/bundles/tetragon-vm/bin/tetragon; sudo install -m 0755 /tmp/sysarmor-fake-tetra /opt/sysarmor/agent/bundles/tetragon-vm/bin/tetra; sudo install -m 0644 /tmp/sysarmor-fake-manifest.json /opt/sysarmor/agent/bundles/tetragon-vm/manifest.json; sudo install -m 0644 /tmp/sysarmor-managed-vm-policy.yaml /etc/sysarmor/policies/sysarmor-managed-vm.yaml; sudo install -m 0644 /tmp/sysarmor-managed-vm-agent.yaml /etc/sysarmor/agent.yaml; sudo systemctl daemon-reload; sudo systemctl enable sysarmor-agent >/dev/null" >/dev/null
...
wait_contains() {
  ...
  until "$@" >"$out" 2>"$out.err" && grep -Fq "$needle" "$out"; do
    if (( SECONDS >= deadline )); then
      ...
      vagrant ssh node-a -c "find /opt/sysarmor/agent/sensors -maxdepth 5 -type f -o -type l | sort || true" >&2 2>/dev/null || true
      exit 1
    fi
    sleep 1
  done
...
vagrant ssh node-a -c "test -x /opt/sysarmor/agent/sensors/tetragon/current/bin/tetragon && test -x /opt/sysarmor/agent/sensors/tetragon/current/bin/tetra" >/dev/null
