...
grep -Fq 'CGO_ENABLED=0 go build -ldflags "-X main.version=$VERSION" -o dist/bin/sysarmor-agent ./apps/agent/cmd/sysarmor-agent' "$build"
grep -Fq 'CGO_ENABLED=0 go build -ldflags "-X main.version=$VERSION" -o dist/bin/sysarmorctl ./apps/cli/cmd/sysarmorctl' "$build"
