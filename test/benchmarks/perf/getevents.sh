#!/usr/bin/env bash
# Capture a lightweight getevents throughput baseline for the selected ENV.
set -euo pipefail
...
OUT="$RESULTS/perf-getevents.$ENV_NAME.csv"
mkdir -p "$RESULTS"

write_header() {
  if [[ ! -f "$OUT" ]]; then
    echo "env,duration_s,events,eps,cpu_pct,rss_mb,dropped_events" > "$OUT"
  fi
}
...
container_perf() {
  ...
  echo "$ENV_NAME,$DUR,$events,$(awk -v e="$events" -v d="$DUR" 'BEGIN { printf "%.2f", e / d }'),${cpu:-0},${rss:-0},0" >> "$OUT"
}
...
vm_perf() {
  ...
  echo "$ENV_NAME,$DUR,${events:-0},$(awk -v e="${events:-0}" -v d="$DUR" 'BEGIN { printf "%.2f", e / d }'),0,${rss:-0},0" >> "$OUT"
}

write_header
case "$ENV_NAME" in
  container) container_perf ;;
  vm-endpoint|vm-topology) vm_perf ;;
